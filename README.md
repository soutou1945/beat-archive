...
- スマホ向けブックマークレットで、CHUNITHM-NETの表示中一覧をJSONとして保存
...
CHUNITHM-NETのスコア一覧を表示してブックマークレットを実行し、「表示中ページを追加」を押します。
必要な一覧で繰り返した後、「JSONを保存」から取得したファイルをBEAT ARCHIVEへ取り込みます。

このツールは表示中のページだけを読み取り、自動巡回、ログイン情報・Cookieの収集、外部サーバーへのスコア送信は行いません。
